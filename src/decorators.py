...
class command:
    def __init__(
        ...
        *,
        ...
        in_game_only: bool = False,
        allow_alt: bool | None = None,
        register: bool = True,
    ):
        ...
        for name in commands:
            for func in COMMANDS[name]:
                if func.owner_only != owner_only or func.flag != flag:
                    raise ValueError(f"mismatched access levels for {func.name}")
    ...
    def caller(self, wrapper: MessageDispatcher, message: str):
        ...
        if (
            wrapper.public
            and wrapper.target is not channels.Main
            and not (self.flag or self.owner_only)
        ):
        ...
        if self.phases and (
            wrapper.game_state is None or wrapper.game_state.current_phase not in self.phases
        ):
            return
        ...
        wrapper.source.update_account_data(
            self.key, functools.partial(self._thunk, wrapper, message)
        )
    ...
    def _caller(self, wrapper: MessageDispatcher, message: str):
        ...
        logger = logging.getLogger(f"command.{self.name}")
        command_log_args = {
