...
from src.status import add_dying, add_protection, try_exchange, try_misdirection
...
def guard(wrapper: MessageDispatcher, message: str):
    ...
    target = get_target(
        wrapper,
        re.split(" +", message)[0],
        allow_self=config.Main.get("gameplay.safes.guard_self"),
...
def on_night_death_message(evt: Event, var: GameState, victim: User, killer: User | str):
    if killer == "@angel":
        evt.data["key"] = (
            "protected_wolf" if var.role_reveal == "on" else "protected_wolf_no_reveal"
        )
...
def on_get_role_metadata(evt: Event, var: GameState | None, kind: str):
    if kind == "role_categories":
        ...
        evt.data["guardian angel"] = {
            "role": "fallen angel",
            "prefix": "fallen_angel",
            "secondary_roles": {"assassin"},
        }
