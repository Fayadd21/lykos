...
from src.cats import All, Category, Vampire_Team, Wolfteam, get_team
from src.containers import UserDict, UserSet
from src.decorators import command
from src.dispatcher import MessageDispatcher
from src.events import Event, event_listener
from src.functions import get_all_roles, get_players, get_target
from src.gamestate import GameState
from src.messages import messages
from src.random import random
from src.status import add_silent, is_silent, try_misdirection
...
def vg_kill(wrapper: MessageDispatcher, message: str):
    ...
    if target not in TARGETS[wrapper.source]:
        ...
        wrapper.pm(messages[f"vengeful_ghost_{GHOSTS[wrapper.source]}"])
        return
...
def on_team_win(
    evt: Event, var: GameState, player: User, main_role: str, all_roles: set[str], winner: Category
):
...
def on_retribution_kill(evt: Event, var: GameState, victim: User, orig_target: User):
    ...
    if target in GHOSTS:
        ...
        evt.data["message"].append(
            messages["retribution_totem_night_banish"].format(victim, target)
        )
...
def on_chk_nightdone(evt: Event, var: GameState):
    ...
    evt.data["nightroles"].extend(
        [p for p in GHOSTS if GHOSTS[p][0] != "!" and TARGETS.get(p, None)]
    )
...
def on_transition_night_end(evt: Event, var: GameState):
    ...
    for v_ghost, who in GHOSTS.items():
        ...
        v_ghost.send(
            messages["vengeful_ghost_notify"].format(who),
...
def on_revealroles(evt: Event, var: GameState):
    if GHOSTS:
        ...
        for ghost, team in GHOSTS.items():
            ...
            if team[0] == "!":
                to_send.append(
                    messages["vg_driven_away"].format()
                )  # call .format() so it's actually a str
            to_send.append(messages["vg_against"].format(team.lstrip("!")))
            glist.append("{} ({})".format(ghost, ", ".join(to_send)))
