...
def on_new_role(evt: Event, var: GameState, player: User, old_role: str | None):
    if (
        evt.data["role"] == "wolf"
        and old_role == "wild child"
        and evt.params.inherit_from
        and "wild child" in get_all_roles(var, evt.params.inherit_from)
    ):
        evt.data["role"] = "wild child"
    ...
    for child in get_all_players(var, ("wild child",)):
        if IDOLS.get(child) is player:
            if death_triggers:
                ...
                change_role(
                    var, child, get_main_role(var, child), "wolf", message="wild_child_idol_died"
                )
    ...
    if role == "wild child" and user not in get_players(var, get_wolfchat_roles()):
        if user in IDOLS:
            evt.data["special_case"].append(
                messages["wild_child_revealroles_picked"].format(IDOLS[user])
            )
        else:
...
def on_update_stats(
    evt: Event, var: GameState, player: User, main_role: str, reveal_role: str, all_roles: set[str]
):
