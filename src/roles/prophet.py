...
@command(
    "pray", chan=False, pm=True, playing=True, silenced=True, phases=("night",), roles=("prophet",)
)
def pray(wrapper: MessageDispatcher, message: str):
    ...
    people = set(get_all_players(var, (role,))) | {
        p for p, r in amn_roles.items() if p in pl and r == role
    }
    if len(people) == 0:
