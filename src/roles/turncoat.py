...
from src.cats import Category, Vampire_Team, Village, Wolfteam, get_team
from src.containers import UserDict, UserSet
from src.decorators import command
from src.dispatcher import MessageDispatcher
from src.events import Event, event_listener
from src.functions import get_all_players, get_players, match_role
...
def change_sides(
    wrapper: MessageDispatcher, message: str, sendmsg=True
):  # is sendmsg useful at all?
    ...
    if role == "turncoat" and user in TURNCOATS:
        if TURNCOATS[user][0] == "none":
            ...
            evt.data["special_case"].append(
                messages["turncoat_revealroles"].format(TURNCOATS[user][0])
            )
