...
@command(
    "curse", chan=False, pm=True, playing=True, silenced=True, phases=("night",), roles=("warlock",)
)
def curse(wrapper: MessageDispatcher, message: str):
    ...
    send_wolfchat_message(
        var,
        wrapper.source,
        messages["curse_success_wolfchat"].format(wrapper.source, orig),
        {"warlock"},
        role="warlock",
        command="curse",
    )
...
@command(
    "pass", chan=False, pm=True, playing=True, silenced=True, phases=("night",), roles=("warlock",)
)
def pass_cmd(wrapper: MessageDispatcher, message: str):
    ...
    send_wolfchat_message(
        wrapper.game_state,
        wrapper.source,
        messages["warlock_pass_wolfchat"].format(wrapper.source),
        {"warlock"},
        role="warlock",
        command="pass",
    )
...
def retract(wrapper: MessageDispatcher, message: str):
    ...
    send_wolfchat_message(
        wrapper.game_state,
        wrapper.source,
        messages["warlock_retract_wolfchat"].format(wrapper.source),
        {"warlock"},
        role="warlock",
        command="retract",
    )
