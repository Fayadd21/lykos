...
from src.functions import get_all_players, get_main_role, get_players, get_reveal_role, get_target
...
def setup_variables(
    rolename: str, *, hit: float, headshot: float, explode: float, multiplier: float
):
    GUNNERS: UserDict[User, int] = UserDict()
    _rolestate[rolename] = {"GUNNERS": GUNNERS}
    ...
    def shoot(wrapper: MessageDispatcher, message: str):
        ...
        target = get_target(
            wrapper, re.split(" +", message)[0], not_self_message="gunner_target_self"
        )
        if not target:
            return
        ...
        if shoot_evt.data["hit"]:
            ...
            if realrole in Wolf and shoot_evt.data["kill"]:
                protected = try_protection(
                    var, target, wrapper.source, rolename, reason="gunner_victim"
                )
                if protected is not None:
                    ...
                    add_dying(
                        var,
                        target,
                        killer_role=get_main_role(var, wrapper.source),
                        reason="gunner_victim",
                        killer=wrapper.source,
                    )
                    kill_players(var)
            ...
            wrapper.send(
                messages[to_send].format(wrapper.source, get_reveal_role(var, wrapper.source))
            )
            add_dying(
                var, wrapper.source, killer_role="villager", reason="gunner_suicide"
    ...
    @event_listener("send_role", listener_id=f"gunners.<{rolename}>.on_send_role")
    def on_send_role(evt: Event, var: GameState):
        for gunner in get_all_players(var, (rolename,)):
            if GUNNERS[gunner] or var.always_pm_role:
                gunner.send(messages[f"{rolename}_notify"].format(GUNNERS[gunner]))

    @event_listener("del_player", listener_id=f"gunners.<{rolename}>.on_del_player")
    def on_del_player(
        evt: Event, var: GameState, victim: User, all_roles: set[str], death_triggers: bool
    ):
        if not death_triggers:
            return
        if (
            GUNNERS.get(victim)
            and rolename in all_roles
            and evt.params.killer_role == "wolf"
            and evt.params.reason == "night_kill"
        ):
            if random.random() * 100 < config.Main.get("gameplay.gunner_wolf.kills_attacker"):
                ...
                if wolves:
                    ...
                    if event.data["hit"] and event.data["kill"]:
                        protected = try_protection(
                            var, shot, victim, rolename, "gunner_overnight_fail"
                        )
                        if protected is not None:
                            channels.Main.send(*protected)
                        else:
                            ...
                            channels.Main.send(
                                messages[to_send].format(victim, shot, get_reveal_role(var, shot))
                            )
                            add_dying(
                                var,
                                shot,
                                killer_role=evt.params.main_role,
                                reason="assassin",
                                killer=victim,
                            )
                    elif event.data["hit"]:
            ...
            if (
                config.Main.get("gameplay.gunner_wolf.steals_gun")
                and GUNNERS[victim]
                and "wolf gunner" in _rolestate
            ):
                ...
                if possible:
                    ...
                    _rolestate["wolf gunner"]["GUNNERS"][looter] = (
                        _rolestate["wolf gunner"]["GUNNERS"].get(looter, 0) + 1
                    )
                    del GUNNERS[victim]
                    var.roles["wolf gunner"].add(looter)
                    looter.send(messages["wolf_gunner"].format(victim))

    @event_listener("myrole", listener_id=f"gunners.<{rolename}>.on_myrole")
    ...
    @event_listener("revealroles_role", listener_id=f"gunners.<{rolename}>.on_revealroles_role")
    ...
    @event_listener("reset", listener_id=f"gunners.<{rolename}>.on_reset")
    def on_reset(evt: Event, var: GameState):
        GUNNERS.clear()

    @event_listener("new_role", listener_id=f"gunners.<{rolename}>.on_new_role")
    ...
    @event_listener("gun_chances", listener_id=f"gunners.<{rolename}>.on_gun_chances")
