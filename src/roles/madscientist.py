...
def on_del_player(
    ...
    kill1 = prots1 is None and add_dying(
        var, target1, killer_role="mad scientist", reason="mad_scientist", killer=player
    )
    kill2 = (
        prots2 is None
        and target1 is not target2
        and add_dying(
            var, target2, killer_role="mad scientist", reason="mad_scientist", killer=player
        )
    )
...
def on_myrole(evt: Event, var: GameState, user: User):
    if user in var.roles["mad scientist"]:
        ...
        evt.data["messages"].append(
            messages["mad_scientist_myrole_targets"].format(target1, target2)
        )
...
def on_revealroles(evt: Event, var: GameState, user: User, role: str):
    if role == "mad scientist":
        ...
        evt.data["special_case"].append(
            messages["mad_scientist_revealroles_targets"].format(target1, target2)
        )
