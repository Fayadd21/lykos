...
from src.cats import Hidden_Eligible, Neutral, Win_Stealer, all_teams, role_order
from src.containers import UserDict
from src.events import Event, event_listener
from src.functions import change_role, get_all_players
...
def on_investigate(evt: Event, var: GameState, actor: User, target: User, spy_role: str):
    if evt.data["role"] == "amnesiac" and spy_role in (
        "augur",
        "detective",
        "investigator",
        "sorcerer",
...
def update_amnesiac(evt: Event, var: GameState, player: User, old_role: str | None):
    # FIXME: exchange totem messes with gameplay.hidden.amnesiac (the new amnesiac is no longer hidden should they die)
    if (
        evt.params.inherit_from is not None
        and evt.data["role"] == "amnesiac"
        and old_role != "amnesiac"
    ):
...
def on_revealing_totem(evt: Event, var: GameState, user: User, role: str):
    if (
        role not in get_blacklist(var)
        and not config.Main.get("gameplay.hidden.amnesiac")
        and var.original_roles["amnesiac"]
    ):
        global STATS_FLAG
        STATS_FLAG = True
    if role == "amnesiac":
