...
from src.status import add_dying, is_silent, try_exchange, try_misdirection, try_protection
...
def on_del_player(
    ...
    if death_triggers and "assassin" in all_roles and player in TARGETED:
        ...
        if target in get_players(var):
            ...
            channels.Main.send(
                messages[to_send].format(player, target, get_reveal_role(var, target))
            )
            add_dying(
                var, target, killer_role=evt.params.main_role, reason="assassin", killer=player
            )
