...
PROTECTIONS: DefaultUserDict[User, DefaultUserDict[User | None, list[ProtectionEntry]]] = (
    DefaultUserDict(lambda: DefaultUserDict(list))
)
...
def try_protection(
    var: GameState, target: User, attacker: User | None, attacker_role: str, reason: str
) -> list[str] | None:
    """Attempt to protect the player, and return a list of messages or None."""
    prots: list[tuple[User | None, ProtectionEntry]] = []
    ...
    if (
        not try_evt.dispatch(var, target, attacker, attacker_role, reason)
        or not try_evt.data["protections"]
    ):
        return None
...
def get_all_protections(
    var: GameState, target: User, scope: Category | set[str] = All
    ...
    for protector, entries in list(PROTECTIONS[target].items()):
        for entry in entries:
            if scope & entry.scope:
                ...
                evt.dispatch(
                    var, target, attacker, attacker_role, protector, entry.protector_role, reason
                )
                if evt.data["remove"]:
                    PROTECTIONS[target][protector].remove(entry)
